...
def edit_book(book_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash('The Book Has Been Edited Successfully And It Has Been Added To The End Of The Catalog')
        return redirect(url_for('main.display_books'))
...
def create_book(pub_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash('Book Has Been Successfully Creates And Has Been Added To The End Of The Catalog')
